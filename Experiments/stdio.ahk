...
;                 Improvements by Lexikos and drifter, see http://www.autohotkey.com/forum/topic34517.html
;
; Script Function:
...
DESCRIPTION
   ...
   because the FileAppend command does not currently support writing to consoles.  
   Since v1.0.48.04, buffering is disabled for stdout, so FileAppend can be used.
RETURN VALUE
...
printf(str)
{
   if (hStdout := DllCall("GetStdHandle", "int", -11))
      return DllCall("WriteFile", "uint", hStdout, "uint", &str, "uint", strlen(str), "uint*", BytesWritten, "uint", NULL)
}
