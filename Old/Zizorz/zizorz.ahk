...
IncludeImages := "exit.ico|filedir.ico|key.ico|help.ico|notification.ico|save.ico|settings.ico|shortcut.ico|zizorz.ico|zizorz_header.jpg"
...
If (Notification != 0)
   NotifyID := Notify("Zizorz:", "Hold a modifier and drag to capture`n[+Ctrl] Copy`n[+Shift] File`n[+Win]Capture window", 8,, "data\img\Zizorz.ico")
...
Hotkey, *+LButton, SaveClip ; Shift (File)
Hotkey, F1, Settings
Hotkey, F2, Settings
...
GuiHelp:
MsgBox, 32, Zizorz help, Zizorz is a tool to create`, save & upload images quickly and intuitively`, making it easier to create & share content with others.`nZizorzâ„¢ was made by Simon StrÃ¥lberg in 2011 using Autohotkey (http://www.autohotkey.com)`n`nTo capture an area`, hold and drag the left mouse button. To capture a window`, hold the windows (#) key and leftclick. Depending on what modifier you use (you must use one)`, you can achieve one of following two options:`n`n[ + Ctrl ] Copy to clipboard`n[ + Shift ] Save as a file in the pre-chosen folder`n`nIn case 3`, the link to the URL respectively file folder will be in your clipboard.`n`nZizorz will not run in the background`, but instead exits when finished`, so it doesn't require memory when not used. Therefore`, it is recommended that you launch Zizorz using an applauncher such as Appifyerâ„¢.
return
...
SoundPlay, data\sounds\click.wav
MsgBox, 65, Shortcut created, Created a shortcut for Zizorzâ„¢ on your desktop.`nAdded the default hotkey Ctrl+Alt+Z to launch Zizorz.`nYou may change the hotkey by editing the shortcut.`n`nTip: If you use Appifyerâ„¢ (made by the same author as Zizorz)`, you can define your own hotkey for any file or app. Do you want to check out Appifyer.com?
IfMsgBox, OK
