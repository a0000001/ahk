...
~!2::
	if(WinExist("ahk_class TfcForm")) {
		...
		; ; Determine if the window is actually minimized, as FC likes to pretend it's not when it is.
		; WinGet, fcStyle, Style, ahk_class TfcForm
		; isMinimized := NOT (fcStyle & 0x10000000)
		
		; ; Window is now activated, but not showing (FC is a little special...) - do a little alt+tab, alt+shift+tab jig.
		; if(isMinimized) {
			; ; Find and double-click the FC tray icon.
			; doubleClickTrayIcon("FreeCommander.exe")
			
			; ; ; ; Below used if normally minimized instead of tray-minimized.
			; ; ; Activate the window.
			; ; WinActivate
			
			; ; ; Otherwise, alt+tab gets annoyed - release the alt that triggered the hotkey.
			; ; Send, {Blind}{Alt Up}
